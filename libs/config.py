...
DEFAULTS: Dict[str, Dict[str, Any]] = {
    "YTCM": {
        "DBPath": "~/.local/share/ytcm/ytcm.db",
...
def _get_config(override_cfg_file: Optional[str] = None) -> configparser.ConfigParser:
    ...
    default_cfg_file = os.path.expanduser(config_dir + "/ytcm/ytcm.conf")
...
class Config:
    ...
    def __init__(self, override_cfg_file: Optional[str] = None) -> None:
        ...
        self._config = config
        self.download_dir = os.path.expanduser(config["YTCM"]["DownloadDir"])
        self.db_path = os.path.expanduser(config["YTCM"]["DBPath"])
        self.quickselect_alphabet = set(config["YTCM"]["alphabet"])
        ...
        self.watcherdir = os.path.expanduser(config["YTCM"]["watcherdir"])
